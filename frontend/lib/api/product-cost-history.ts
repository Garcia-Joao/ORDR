...
function normalizeRow(row: any): ProductCostHistoryRow {
  ...
  return {
    ...
    referenceCost: toNullableNumber(row?.newReferenceCost ?? row?.referenceCost),
...
function buildProductSummaries(
  ...
  for (const row of rows) {
    ...
    current.deltaPercent =
      current.firstCost > 0 ? (current.deltaCost / current.firstCost) * 100 : null
...
function buildCategorySummaries(
  ...
  for (const row of rows) {
    ...
    current.averageCost = current.changes > 0 ? current.totalCost / current.changes : 0
...
function normalizeProductSummary(
  ...
  const normalizedHistory = asArray<any>(summary?.history).map((row: any) =>
    normalizeRow(row)
  )

  return {
    productId: String(summary?.productId ?? built?.productId ?? ''),
    productName: String(summary?.productName ?? built?.productName ?? 'Produto'),
    ...
    deltaCost: toNumber(summary?.deltaCost ?? summary?.delta ?? built?.deltaCost),
    deltaPercent: toNullableNumber(summary?.deltaPercent ?? built?.deltaPercent),
    lastChangedAt: String(
      summary?.lastChangedAt ?? built?.lastChangedAt ?? new Date().toISOString()
    ),
    history: normalizedHistory.length > 0 ? normalizedHistory : productRows,
  }
}
...
function normalizeCategorySummary(
  ...
  return {
    ...
    productCount: toNumber(
      summary?.productCount ?? summary?.productsChanged ?? summary?.products ?? 0
    ),
