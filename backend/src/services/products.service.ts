    ...
    return {
      ...
      ingredientProduct: ingredient
        ? {
            ...
            simpleCost:
              ingredient.simpleCost == null ? null : Number(ingredient.simpleCost),
            referenceQuantity:
              ingredient.referenceQuantity == null
                ? null
                : Number(ingredient.referenceQuantity),
            referenceCost:
              ingredient.referenceCost == null ? null : Number(ingredient.referenceCost),
            unitContentQuantity:
              ingredient.unitContentQuantity == null ? null : Number(ingredient.unitContentQuantity),
  ...
  return {
    ...
    variationGroups: (product.variationGroups ?? [])
      ...
      options: (group.options ?? []).map((option: any) => {
        const normalizedOptionRecipeItems = (option.recipeItems ?? []).map((item: any) => {
          const ingredient = item.ingredientProduct
          const referenceQuantity = Number(ingredient?.referenceQuantity ?? 0)
          const referenceCost = Number(ingredient?.referenceCost ?? 0)
        ...
        return {
          ...
          referenceQuantity:
            option.referenceQuantity == null ? null : Number(option.referenceQuantity),
          referenceCost:
            option.referenceCost == null ? null : Number(option.referenceCost),
          recipeCost,
          recipeItems: normalizedOptionRecipeItems,
          environmentPrices: (option.environmentPrices ?? []).map((item: any) => ({
            ...item,
  ...
  for (const item of recipeItems) {
    ...
    const ingredient = await prisma.product.findFirst({
      where: {
        ...
        active: true,
        deletedAt: null,
        OR: [{ trackStock: true }, { isStockOnly: true }, { unlimitedStock: true }],
      },
    })
...
export async function createProduct(data: CreateProductInput, userId: string) {
  ...
  const createPayload = {
    ...
    referenceCost:
      data.referenceCost == null ? null : new Prisma.Decimal(data.referenceCost),
    unitContentQuantity:
      data.unitContentQuantity == null ? null : new Prisma.Decimal(data.unitContentQuantity),
...
async function syncProductVariationGroups(params: {
  ...
  for (const [groupIndex, group] of variationGroups.entries()) {
    ...
    const matchedExistingGroup = group.id
      ? existingGroups.find((existingGroup: any) => existingGroup.id === group.id)
      : existingGroups.find(
          (existingGroup: any) =>
    ...
    for (const [optionIndex, option] of incomingOptions.entries()) {
      ...
      const matchedExistingOption = option.id
        ? existingOptions.find((existingOption: any) => existingOption.id === option.id)
        : existingOptions.find(
            (existingOption: any) =>
              normalizeName(existingOption.name) === normalizeName(cleanOptionName)
          )
      ...
      data: {
        ...
        stockQuantity:
          data.stockQuantity === undefined
            ...
            : data.stockQuantity,
        minStock:
          data.minStock === undefined ? existingProduct.minStock : data.minStock,
        ...
        stockUnit:
          data.stockUnit === undefined ? existingProduct.stockUnit : data.stockUnit,
        referenceQuantity:
...
export async function deleteProduct(productId: string, companyId: string, userId: string) {
  const existingProduct = await prisma.product.findFirst({
    where: {
      id: productId,
      companyId,
